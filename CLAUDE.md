...
- **Click a space in the menu bar to switch to it** — asked 2026-08-12. Fixed same day (v1.24.1 permission request + v1.24.2 diagnostics; root cause: unsigned builds get per-binary TCC grants, needed a fresh manual Accessibility add for the installed binary). **Signed off by Conner 2026-08-12 ("it works").**
- **Known caveat (unsigned builds)**: every new CI build has a new code fingerprint, so macOS drops the Accessibility grant on every update. After installing a new build: System Settings → Privacy & Security → Accessibility → remove Spaceman with −, re-add with +, toggle on, restart the app. Durable fix (proposed, not yet approved): re-sign CI artifacts locally with a stable self-signed identity at install time.
- **Use the fork's own bundle ID (upstream author's request, email 2026-09-04)** — asked 2026-09-14. Session believes fixed in v1.25.0 (`io.github.connerstobie.Spaceman`; settings migrated, verified on Conner's Mac: `migratedFromUpstreamBundleID=1`, names and switching mode present in the new domain). Check: `defaults read /Applications/Spaceman.app/Contents/Info.plist CFBundleIdentifier` and spaces still named after relaunch. Reply to René drafted in Gmail, not sent. **Awaiting Conner's sign-off.**
- **Two-row space names too small to read** — asked 2026-09-17. Session believes fixed in v1.26.0: two-row layout gains `.extraLarge` (12pt) and `.enormous` (13pt) entries sized for the 34pt notched menu bar (measured on Conner's 16"); his stored `iconSize=4` now maps to the 12pt entry, scrolling up on the icon gives 13pt. 14pt deliberately left out: ~249pt wide is at the estimated free-width limit and with `autoShrink=false` an overflowing icon vanishes. CI build downloaded to `~/Downloads/Spaceman-1.26.0/`; not yet installed (needs the Accessibility re-grant dance). Check: names visibly larger after install. **Awaiting Conner's sign-off.**
- **Never show numbers; names in full on one line** — asked 2026-09-17 after the 12pt single row kept auto-shrinking to numbers. Session believes fixed in v1.27.0: auto-shrink (`ShrinkLevel`, `ShrinkOverrides`, the `autoShrink` toggle) removed; replaced by fit-to-width (`fittedSize` in AppDelegate, `IconSize.nextSmaller(twoRows:)`) that only steps the icon size down, keeps it across space switches, and retries the user's size on refresh/topology/unlock. macOS still hides a status item that does not fit at the smallest size; that is the only remaining failure mode. Check: names never replaced by numbers; switching spaces does not blink. **Awaiting Conner's sign-off.**
